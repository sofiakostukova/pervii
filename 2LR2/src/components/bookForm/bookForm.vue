<template>
  <div :class="$style.root">
    ...
    <div :class="$style.item">
      ...
      <select v-model="form.author" :class="$style.select" name="author" id="author">
        <option v-for="({ author, id  }) in authorList" :key="id" :value="author">
          {{ author }}
        </option>
      </select>
    </div>
...
import { selectItemById, fetchItems } from '@/store/books/selectors';
import { selectItems as selectauthors, fetchItems as fetchauthors } from '@/store/authors/selectors';
...
export default {
  ...
  setup(props, context) {
    ...
    const authorList = computed(() => selectauthors(store))
    const form = reactive({
      id: '',
      name: '',
      author: '',
    ...
    watchEffect(() => {
      const book = selectItemById( store,  props.id );
    ...
    return {
      authorList,
      form,
      isValidForm: computed(() =>  !!(form.name && form.description && form.price && form.author)),
      onClick: () => {
